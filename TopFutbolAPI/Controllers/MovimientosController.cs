...
namespace TopFutbolAPI.Controllers
{
    ...
    public class MovimientosController : ControllerBase
    {
        ...
        public async Task<ActionResult<MovimientoDTO>> PostMovimiento(MovimientoDTO movimientoDTO)
        {
            ...
            // Actualizar el balance de la sede
            var balanceActual = await _context.BalancesAlumnos
                .Where(b => b.IdSede == movimientoDTO.IdSede && b.Fecha.Date == movimientoDTO.Fecha.Date)
                .FirstOrDefaultAsync();
            
            if (balanceActual == null)
            {
                // Si no existe un balance para esta fecha y sede, crear uno nuevo
                balanceActual = new BalanceAlumno
                {
                    IdSede = movimientoDTO.IdSede,
                    IdAlumno = movimientoDTO.IdAlumno,
                    Fecha = movimientoDTO.Fecha,
                ...
                _context.BalancesAlumnos.Add(balanceActual);
            }
            
            // Actualizar el valor del balance según el tipo de movimiento
            if (movimiento.IdTipo == 1) // Cargo (ingreso)
            {
                balanceActual.Valor += movimiento.Valor;
            }
        ...
        public async Task<IActionResult> PutMovimiento(int id, MovimientoDTO movimientoDTO)
        {
            ...
            // Guardar los valores originales para actualizar el balance
            ...
            try
            {
                // Actualizar el balance de la sede original (revertir el movimiento)
                var balanceOriginal = await _context.BalancesAlumnos
                    .Where(b => b.IdSede == idSedeOriginal && b.Fecha.Date == fechaOriginal.Date)
                    .FirstOrDefaultAsync();
                ...
                if (balanceOriginal != null)
                {
                    ...
                    {
                        balanceOriginal.Valor -= valorOriginal;
                    }
                ...
                // Actualizar el balance de la nueva sede
                var balanceNuevo = await _context.BalancesAlumnos
                    .Where(b => b.IdSede == movimientoDTO.IdSede && b.Fecha.Date == movimientoDTO.Fecha.Date)
                    .FirstOrDefaultAsync();
                
                if (balanceNuevo == null)
                {
                    // Si no existe un balance para esta fecha y sede, crear uno nuevo
                    balanceNuevo = new BalanceAlumno
                    {
                        IdSede = movimientoDTO.IdSede,
                        IdAlumno = movimientoDTO.IdAlumno,
                        Fecha = movimientoDTO.Fecha,
                    ...
                    _context.BalancesAlumnos.Add(balanceNuevo);
                }
                
                // Aplicar el efecto del nuevo movimiento
                if (movimiento.IdTipo == 1) // Cargo (ingreso)
                {
                    balanceNuevo.Valor += movimiento.Valor;
                }
                else if (movimiento.IdTipo == 2) // Abono (egreso)
                {
                    balanceNuevo.Valor -= movimiento.Valor;
        ...
        public async Task<IActionResult> DeleteMovimiento(int id)
        {
            ...
            var balance = await _context.BalancesAlumnos
                .Where(b => b.IdSede == movimiento.IdSede && b.Fecha.Date == movimiento.Fecha.Date)
                .FirstOrDefaultAsync();
            ...
            if (balance != null)
            {
                ...
                if (movimiento.IdTipo == 1) // Cargo (ingreso)
                {
                    balance.Valor -= movimiento.Valor;
                }
                else if (movimiento.IdTipo == 2) // Abono (egreso)
                {
                    balance.Valor += movimiento.Valor;
